...
const SHADOW_DIR = 'C:\\dev\\web-terminal';
...
const URLS = {
  office: 'http://***REDACTED-IP***:7784',
  home:   'http://***REDACTED-IP***:7785',
  xps:    'http://***REDACTED-IP***:7786',
};
const TOKENS = {
  office: '***REDACTED-TOKEN***',
  home:   '***REDACTED-TOKEN***',
  xps:    '***REDACTED-TOKEN***',
};
const PASSHASH = '$scrypt$***REDACTED***$***REDACTED-TOKEN******REDACTED-TOKEN***';
...
objShell.Environment("PROCESS")("WT_WORKER_PIPE") = "\\\\.\\pipe\\web-terminal-${PIPE_SUFFIX}"
objShell.Run "node monitor.js", 0, False
`;
fs.writeFileSync(path.join(SHADOW_DIR, 'start-server.vbs'), vbs);
console.log('wrote start-server.vbs');

// Step 8: Ensure clean state on launch (kill any prior shadow and clear logs)
try {
  cp.execSync(`powershell -NoProfile -Command "Get-CimInstance Win32_Process -Filter \\"Name='node.exe'\\" | Where-Object { $_.CommandLine -like '*web-terminal*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"`, { stdio: 'inherit', windowsHide: true });
} catch (e) { /* ignore */ }
...
cp.spawn('wscript.exe', ['start-server.vbs'], { cwd: SHADOW_DIR, detached: true, stdio: 'ignore', windowsHide: true }).unref();
console.log('launched shadow via wscript');
console.log('shadow config: port=' + PORT + ' pipe=\\\\.\\pipe\\web-terminal-' + PIPE_SUFFIX + ' url=' + URLS[ROLE]);
