...
const TARGETS = {
  home: { url: 'http://***REDACTED-IP***:7785', token: '***REDACTED-TOKEN***', pipe: 'home' },
  xps:  { url: 'http://***REDACTED-IP***:7786',  token: '***REDACTED-TOKEN***', pipe: 'xps' },
};
...
async function exec(cmd) {
  console.log('$ ' + cmd);
  const r = await httpJson('POST', T.url + '/api/exec', T.token, { command: cmd, cwd: 'C:\\dev\\web-terminal', timeout: 120000 });
  if (r.status !== 200) {
...
(async () => {
  ...
  await exec('git -C C:/dev/web-terminal fetch origin feature/direct-terminal');
  await exec('git -C C:/dev/web-terminal reset --hard origin/feature/direct-terminal');

  // Step 2: rewrite start-server.vbs so WT_LATENCY_DEBUG=1 is exported before monitor.js
  ...
  const vbs = `' Shadow cluster launcher (${ROLE}) - isolated pipe, own config/state.\r\n` +
              ...
              `objShell.Environment("PROCESS")("WT_WORKER_PIPE") = "\\\\.\\pipe\\web-terminal-${T.pipe}"\r\n` +
  ...
  const psCmd = `powershell -NoProfile -Command "[IO.File]::WriteAllText('C:/dev/web-terminal/start-server.vbs', [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('${payload}')))"`;
  await exec(psCmd);

  // Step 3: restart — kill existing shadow monitor/server/worker then relaunch via wscript.
  // Only kill node procs whose cmdline includes 'web-terminal', NOT generic node.
  const killPs = `powershell -NoProfile -Command "Get-CimInstance Win32_Process -Filter \\"Name='node.exe'\\" | Where-Object { \\$_.CommandLine -like '*web-terminal*' } | ForEach-Object { Stop-Process -Id \\$_.ProcessId -Force -ErrorAction SilentlyContinue }"`;
  await exec(killPs);
  ...
  const restartCmd = `powershell -NoProfile -Command "Start-Sleep -Seconds 2; cd C:/dev/web-terminal; wscript.exe start-server.vbs"`;
  // Fire-and-forget via spawn so /api/exec doesn't wait on it
  await exec(`powershell -NoProfile -Command "Start-Process -FilePath powershell -ArgumentList '-NoProfile -Command &{ Start-Sleep -Seconds 2; Set-Location C:/dev/web-terminal; wscript.exe start-server.vbs }' -WindowStyle Hidden"`);
