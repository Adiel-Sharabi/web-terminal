...
- **AiTerminal** — a native [Flutter](https://flutter.dev) client for the same server, living in [`ai-terminal/`](ai-terminal/). Talks to the identical REST/WebSocket API, so it is a client choice, not a fork. See [Companion App](#companion-app--aiterminal) below for what it adds over the browser.
...
| `enableRemoteExec` | Yes | Enable `POST /api/exec` (arbitrary shell commands for any valid bearer-token holder). Default **false**; the route returns 404 while disabled. See the `/api/exec` note under [Security](#security) before turning it on. |
...
|---|---|
| [README.md](README.md) | This file — features, install, configuration, operations |
| [AGENTS.md](AGENTS.md) | **Point an AI agent at this repo and it can install, verify and operate the server** — install steps, the hot-vs-cold restart decision, the Windows traps, and the never-do list |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Technical walkthrough of the three-process model, IPC, and data flow |
| [CONTRIBUTING.md](CONTRIBUTING.md) | Dev setup, the mandatory pre-commit gates, and the PR acceptance gate |
| [SECURITY.md](SECURITY.md) | Threat model, supported deployments, and private vulnerability reporting |
| [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) | Community expectations |
| [CLAUDE.md](CLAUDE.md) | Instructions for AI coding agents working in this repo — the invariants a change must not break |
| [ENGINEERING_STANDARDS.md](ENGINEERING_STANDARDS.md) | The working agreement: SSOT, root-cause-first, minimal blast radius |
| [PEER_PROTOCOL.md](PEER_PROTOCOL.md) | Contract for the localhost peer relay two agents use to consult each other |
| [HOT-RELOAD-DESIGN.md](HOT-RELOAD-DESIGN.md) | How `server.js` restarts without losing PTY sessions |
| [CLUSTER-MONITOR.md](CLUSTER-MONITOR.md) | The cluster latency/health monitor and what its measurements mean |
| [COMPANION-APP-DESIGN.md](COMPANION-APP-DESIGN.md) | Companion app architecture and the push/privacy model |
| [COMPANION-APP-UI-SPEC.md](COMPANION-APP-UI-SPEC.md) | Companion app UI specification |
| [VOICE-INTEGRATION-DESIGN.md](VOICE-INTEGRATION-DESIGN.md) | Read-aloud design and the filtering rules that make speech usable |
| [ai-terminal/WINDOWS-BUILD.md](ai-terminal/WINDOWS-BUILD.md) | Building and installing the Android + Windows companion |
| [ai-terminal/third_party/xterm/README-PATCH.md](ai-terminal/third_party/xterm/README-PATCH.md) | The vendored xterm patch and how to verify it |
| [PLANS.md](PLANS.md) | Prioritized backlog of future work |
...
| `lib/submit-frames.js` | Pure rules for the two turn-lifecycle keys on the PTY input path: `splitTrailingCr` (hold the submit CR back so a TUI that folds a read into a paste sees Enter, not a newline), `endsBracketedPaste` (which frame leaves a paste open to the *next* frame's CR — the images-only submit) and `isEscapeKey` (a *lone* `0x1b` is the interrupt; `ESC [ A` is an arrow) |
...
| `scripts/install-codex-notify.js` | Writes the three `[tui]` keys that turn that channel on (sibling of `install-statusline.js`). `--check` reports without changing anything. **Run by `scripts/cold-restart.ps1` on every deploy** — it used to be a manual per-machine step and was therefore missing on all three boxes at once (#82) |
...
| `ai-terminal/` | **AiTerminal** — the native Flutter client (Android + Windows) for this server: chat/terminal lenses, session browser, FCM push, and the interactive-question overlay. Talks to the same REST/WebSocket API; built with `flutter`. Its own Dart tests run via `flutter test`, separate from the Playwright suite. See [Companion App](#companion-app--aiterminal) and [`ai-terminal/README.md`](ai-terminal/README.md) |
