...
- **The companion's session header has an explicit width budget (#74)** — `AppBar` lays its `actions` out at their intrinsic width **first** and hands the title whatever is left, so the title was the only flexible child and absorbed every control's shortfall. On a phone that collapsed a session name to `● Lo…` — two characters — and each control added over time (the lens toggle, then #40's chrome, then #70's speaker) made it quietly worse. The fix is not to shrink one widget but to invert the relationship: `headerFit()` (pure, unit-tested) reserves the title a floor and **folds controls into the overflow menu in a declared order** — server chip first (it is context, and also in the sidebar), then detach, then read-aloud, with the lens toggle last because switching views has no other one-tap path. A folded control keeps its capability in the overflow, so a narrow screen loses chrome, never function. The property that matters is the invariant, and it is the thing under test: **adding a control later can only make things fold earlier — it can never re-truncate the title**
