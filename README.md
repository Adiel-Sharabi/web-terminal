...
- **Agent-aware prompt submit** — sending a prompt from chat (or typing and hitting Enter in one burst) reaches the worker as a single frame ending in `\r`. Codex's TUI folds a whole read into a *paste*, so that `\r` becomes a newline in its composer and the prompt is typed but never sent; Claude's TUI has no such detector for a plain `text\r` and its clients rely on the frame staying atomic (#44). Each provider therefore declares `submit: { gapMs, crBurstsAsPaste }` in `lib/agents.js`, and `pty-worker.js` — the one place that knows both the agent and the raw bytes — withholds the CR and writes it alone `gapMs` later for the agents that need it, queueing anything that arrives in between so input order holds. A **multi-line** prompt is different: every client sends it as a bracketed paste (`ESC[200~…ESC[201~\r`), and *both* TUIs absorb a CR that lands in the same read as the paste close (measured: paste+CR never submits, the same paste + a `gapMs`-delayed CR submits). So the worker splits that trailing CR for **every** agent when a frame ends in `ESC[201~\r` (`lib/submit-frames.js` `endsWithPasteSubmitCr`) — single-line stays atomic. Clients send the same bytes as before, so every client (web, companion, future) is fixed at once with no release.
...
| `lib/submit-frames.js` | Pure rule for delivering a submit `\r` to a TUI that folds a whole read into a paste (Codex): split the CR off the frame so it lands as Enter, not a newline |
