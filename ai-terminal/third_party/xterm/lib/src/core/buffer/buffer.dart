...
class Buffer {
  ...
  void scrollDown(int lines) {
    for (var i = absoluteMarginBottom; i >= absoluteMarginTop; i--) {
      if (i >= absoluteMarginTop + lines) {
        // WEB-TERMINAL PATCH (#81): was `this.lines[i] = this.lines[i - lines]`,
        // which leaves the line aliased in two slots so the next iteration
        // detaches it. See IndexAwareCircularBuffer.move.
  ...
  void scrollUp(int lines) {
    for (var i = absoluteMarginTop; i <= absoluteMarginBottom; i++) {
      if (i <= absoluteMarginBottom - lines) {
        // WEB-TERMINAL PATCH (#81): see scrollDown.
        this.lines.move(i + lines, i);
      } else {
        this.lines[i] = _newEmptyLine();
      }
  ...
  void deleteLines(int count) {
    ...
    for (var i = 0; i < linesToMove; i++) {
      ...
      // WEB-TERMINAL PATCH (#81): see Buffer.scrollDown. insertLines just above
      // already avoids the alias by going through `lines.swap`; these three sites
