...
class IndexAwareCircularBuffer<T extends IndexedItem> {
  ...
  /// WEB-TERMINAL PATCH (#81) — added, no stock behaviour changed.
  ///
  /// Moves the element at [fromIndex] to [toIndex] as a genuine *move*: the
  /// element ends up attached at its new index and [fromIndex] is left EMPTY.
  ///
  /// This exists because `buffer[to] = buffer[from]` — the obvious way to shift a
  /// line, and what [Buffer.scrollUp], [Buffer.scrollDown] and
  /// [Buffer.deleteLines] used to do — leaves the element referenced by BOTH
  /// slots. [_adoptChild] unconditionally detaches whatever occupied the
  /// destination, so the next iteration of such a loop detaches the very element
  /// the previous one just moved. The result is a buffer full of elements that are
  /// still in [_array] (so they still render) but are detached from it, which
  /// breaks every anchor-based feature — selection above all — and makes a later
  /// [_moveChild] throw on `_owner!`.
  ///
  /// CONTRACT: this deliberately leaves a hole at [fromIndex]. Callers shift over a
  /// contiguous range and then refill the vacated slots, which is exactly what the
  /// three call sites do. Reading an un-refilled slot would throw.
