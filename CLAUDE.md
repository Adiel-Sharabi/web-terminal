...
- **Cold-restart practice — always drive the restart from a different server than the one being restarted.** This isolates the kill logic from the process being killed, so a bug in the kill script can't silently leave the target offline.
  - **Office** — driven from the local canary at `C:\dev\web-terminal` (git worktree of master, port 7683, pipe `\\.\pipe\web-terminal`, creds `canary`/`***REDACTED***`). Start via `wscript C:\dev\web-terminal\start-server.vbs`. Get a bearer token: `POST http://localhost:7683/api/auth/token` with JSON `{user,password,label}`. Then POST the restart payload to `http://localhost:7683/api/exec`.
  - **Home** — driven from Office via `https://***REDACTED***/api/exec` + Home's bearer token from `cluster-tokens.json`.
  - **XPS** — driven from Office via `https://***REDACTED***/api/exec` + XPS's bearer token.
  ...
  - **PS1 kill filter** — match `*C:\dev\web-terminal\server.js*` and `*C:\dev\web-terminal\pty-worker.js*` with `-like` (NOT `-match` — `\p` in `pty-worker.js` breaks PowerShell regex as a malformed `\p{X}` Unicode class). For `monitor.js` (no path in CommandLine), add `-notlike` excludes for `****REDACTED****`, `****REDACTED****`, `*vite*`, `*web-terminal*`, `*web-terminal*`. Use the ParentProcessId of the matched server/worker to identify the matching monitor.
