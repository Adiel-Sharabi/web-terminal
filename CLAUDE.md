...
**Hooks (not used today — but the recorded reason below is WRONG; see #78).** Codex cloned Claude's hook protocol (same event names, payload fields, `exit 2 = block`). This section used to say the blocker was hook *trust* — "hooks are PascalCase TOML in `config.toml`", "only `managed` hooks run unattended", "user-level hooks load as `trust=untrusted` and never execute". **Do not act on that.**

Verified on server-a, 2026-08-02, by reading the machine rather than the docs: `~/.codex/config.toml` contains a `[features]` section whose own comment reads *"Hook engine is gated; without this hooks silently no-op"*, with `hooks = true`. **A feature gate — not trust — is the thing that silently disables hooks**, and it is almost certainly what the original investigation actually hit. Hooks are configured in a user-level **`~/.codex/hooks.json`** in Claude Code's own JSON shape (plus a `statusMessage` field); no such file exists on this machine, which is why nothing fires here today.

**Still unproven, and #78's blocking step:** that `PermissionRequest` / `Stop` / `UserPromptSubmit` / `PostToolUse` / `PostCompact` / `SubagentStop` actually fire unattended, and what their payloads contain. A live `SessionStart` hook is the only positive evidence. **Capture the real payloads before building on them** — the standing rule for Codex is that the docs are not trusted, and that applies to this paragraph too. `SubagentStop` is reported to carry `agent_transcript_path` directly, which would be easier than Claude's `agent-*.meta.json` → `toolUseId` link. `codex exec` runs no hooks at all — only the interactive TUI / `app-server`.

**Why OSC 9 still cannot be retired on its own (#78 item 1):** OSC 9 fires on exactly two occasions — an approval and a finished turn. **There is no turn-start notification, so `working` is unreachable through it by construction.** That is a structural gap hooks would close, not a deployment bug.
