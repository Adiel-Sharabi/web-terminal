...
**Hooks (still not used — and the reason is a HARD blocker, measured 2026-08-02).** Codex cloned Claude's hook protocol (same event names, payload fields, `exit 2 = block`). There are **TWO independent gates, and both are real** — an earlier edit of this file claimed the second one had replaced the first, which is wrong:

1. **The feature gate.** `~/.codex/config.toml` needs `[features] hooks = true`; its own comment reads *"Hook engine is gated; without this hooks silently no-op."* Verified present on all three machines.
2. **Per-definition TRUST, and it is an INTERACTIVE PROMPT THAT BLOCKS STARTUP.** Writing a user-level `~/.codex/hooks.json` (Claude Code's own JSON shape) makes the next `codex` launch stop before the composer renders on:
   > `Hooks need review — 10 hooks are new or changed. Hooks can run outside the sandbox after you trust them. 1. Review hooks  2. Trust all and continue  3. Continue without trusting (hooks won't run)`

   Measured by installing a 10-event logging hook on Home and driving a real PTY: **zero hooks fired**, and the session never reached its composer. Removing `hooks.json` restored normal startup immediately. The count tracks the hook DEFINITIONS ("10 hooks are new or changed"), so trust is bound per definition — changing a command string re-prompts, on every machine.

**Deployment consequence, and it is the whole answer to #78 item 6.** This is the same failure class as the `Update available` nag: an interactive prompt that a worker-spawned PTY has nobody to answer, so **shipping `hooks.json` fleet-wide would wedge every new Codex session** until someone trusts it on each box. A hooks-based status channel therefore cannot ship as "write the file and cold-restart" — the trust answer has to be pre-supplied per machine, or the whole approach abandoned. **This is why OSC 9 was the right channel**, and it substantially vindicates the original "trust is the blocker" note this section once dismissed.

**Still unproven** (the probe never got past the trust prompt, so it captured nothing): whether `PermissionRequest` / `Stop` / `UserPromptSubmit` / `PostToolUse` / `PostCompact` / `SubagentStop` fire unattended once trusted, and what their payloads hold. **Capture them before building on them.** `SubagentStop` is *reported* to carry `agent_transcript_path` directly, which would beat Claude's `agent-*.meta.json` → `toolUseId` link. `codex exec` runs no hooks at all — only the interactive TUI / `app-server`.
