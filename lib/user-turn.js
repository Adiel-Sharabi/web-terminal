...
//
// NO DEPENDENCY THAT CAN CYCLE, and that is not tidiness. The rule started in
...
//
// The one import, `lib/ansi.js`, is a LEAF — it requires nothing at all, so it
// cannot reintroduce the cycle this file was split out to avoid.
...
// Measured over 1012 transcripts / 676 `queued_command` records: 35 carry
// `origin.kind: 'peer'`, and they do not all look alike —
//   * 29 open with `<agent-message from="NAME">`;
//   * 6 open with `<cross-session-message from="uds:\\.\pipe\LOCAL\cc-msg-…"
//     from-name="NAME" from-mode="bypass">`.
// The issue named only the first. The second matters because of WHERE the name
// lives: its `from` is an opaque named-pipe path and the readable name is
// `from-name`, so a `from="…"` read alone labels the bubble with an IPC path.
// Hence the priority order below — `from-name`, then `from`, then the original
// `teammate_id` — and hence ONE tag list rather than a second branch: all three
// wrappers mean the same thing, "another session said this".
const TEAMMATE_ID_RE = /teammate_id="([^"]*)"/;
// ATTRIBUTES ARE READ OFF THE OPENING TAG, never off the whole turn. A teammate
// message carries somebody else's prose, which can quote anything — #192's rule
// that a signature must be anchored, applied to the attribute rather than to the
// branch. Non-global on purpose: a `/g` regex reused with `.exec` is stateful and
// answers differently on alternate calls.
const TEAMMATE_OPEN_RE = /<(?:teammate-message|agent-message|cross-session-message)\b[^>]*>/;
const TEAMMATE_FROM_NAME_RE = /\sfrom-name="([^"]*)"/;
const TEAMMATE_FROM_RE = /\sfrom="([^"]*)"/;
const TEAMMATE_TAG_RE = /<\/?(?:teammate-message|agent-message|cross-session-message)\b[^>]*>/g;
...
// Terminal escapes. Command output is captured off a TERMINAL, so it arrives
// carrying rendering instructions — 47 of the 61 measured turns hold `ESC[2m`
// dim markers IN THE RAW JSONL. Stripped only where the text is known to be
// machine output: a human who pastes terminal output into a prompt typed those
// bytes, and editing their sentence is not this module's job.
//
// BELT-AND-BRACES ON THE PRODUCTION PATH, and worth being accurate about. Every
// caller that reads a transcript comes through `_capTurnText`, which strips
// before `classifyUserTurn` ever sees the text — so in production the escapes
// are already gone and this only fires for a caller handing over raw text (the
// unit tests, and anything future). It is imported rather than re-written for
// exactly that reason: a second copy that almost never runs is a second copy
// nobody notices drifting, and the first draft of it had already drifted — its
// CSI parameter class missed `:` `<` `=` `>`, so a colon-form `ESC[38:5:196m`
// survived a strip that claimed to remove it.
...
function classifyUserTurn(text, opts = {}) {
  ...
  //
  // #250 — `<agent-message>` and `<cross-session-message>` are the shapes a peer's
  // message takes when it is QUEUED mid-turn, and neither was recognised here. The
  // cost was not a mislabelled bubble but an absent one: `lib/transcript.js` had to
  // refuse to emit such a turn at all (its `origin.kind === 'human'` gate), because
  // the only alternative was letting another agent's words through as `human` — a
  // "You" bubble for something you never wrote, which is the teammate-bubble bug
  // this repo has already fixed once. With the signature here, that gate can widen
  // and the classifier decides the bubble, which is the one-owner arrangement every
  // other signature in this file already has.
  if (t.startsWith('Another Claude session sent a message') || t.startsWith('<teammate-message')
      || t.startsWith('<agent-message') || t.startsWith('<cross-session-message')) {
    const tag = (TEAMMATE_OPEN_RE.exec(t) || [''])[0];
    const id = (TEAMMATE_FROM_NAME_RE.exec(tag) || TEAMMATE_FROM_RE.exec(tag) ||
                TEAMMATE_ID_RE.exec(tag) || [])[1] || '';
    const body = t
  ...
  if (t.startsWith('<local-command-stdout')) {
    const body = stripAnsi(t.replace(COMMAND_TAG_RE, ' '))
      .replace(/\s+/g, ' ')
      .trim();
