 ...
 *     The comparison is against BYTES for a Buffer (every caller on the PTY path) and
 *     UTF-16 code units for a string. They agree over the ASCII the threshold was
 *     measured in, and for multi-byte text the byte count is the LARGER of the two — so
 *     the disagreement can only make it wrap sooner, which is the safe direction.
...
function bracketLongBody(head, limit) {
  if (!limit || !head || head.length <= limit) return null;
