...
function scanBackgroundTasks(text) {
  ...
  for (const line of String(text || '').split('\n')) {
    ...
    const looksLikeKill = line.includes('TaskStop') || line.includes('KillShell');
    if (!looksLikeLaunch && !looksLikeBash && !looksLikeKill) continue;
    ...
    for (const b of blocks) {
      ...
      // A task KILLED from the orchestrator gets no `<task-notification>` at all —
      // only this tool call — so without it a stopped build showed as running until
      // the caller's age backstop. Caught against the live API, where a suite I had
      // stopped was still listed. Read off the tool_use (not the result text) so a
      // grep that merely QUOTES "Successfully stopped task: …" can never end one.
      // Any string in the input may be the id; only ids we saw launch can match.
      if (b && b.type === 'tool_use' && (b.name === 'TaskStop' || b.name === 'KillShell')) {
        for (const v of Object.values(b.input || {})) {
          if (typeof v === 'string' && v) finished.add(v);
        }
      }
