...
const { clampPct, resetAtMsFromSeconds } = require('./metrics-common');

// A model id / effort level becomes rendered text in both clients. Cap it at the
// same length the endpoint has always capped pushed strings to.
const LABEL_MAX = 40;
...
function parseStatusPayload(body) {
  ...
  if (!_isRawPayload(body)) {
    return {
      ...
      fiveHResetAt: null,
      model: _label(body.model),
      effort: _label(body.effort),
    };
  }
...
function mergeStatus(prev, next) {
  ...
  const p = prev || {};
  const stable = {
    ctxWindow: next.ctxWindow !== null && next.ctxWindow !== undefined ? next.ctxWindow : (p.ctxWindow ?? null),
    model: next.model || p.model || null,
    effort: next.effort || p.effort || null,
  };
  if (!hasReading(next)) {
    if (!prev) return { ...next, ...stable };
    return { ...p, ...stable }; // keep the stored reading AND its ts
  }
  return {
    ctx: next.ctx,
