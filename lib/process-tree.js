 ...
 */
function snapshotNoWait(now = Date.now(), opts = {}) {
  if (!_cache.procs || now - _cache.at >= SNAPSHOT_TTL_MS) {
    Promise.resolve(snapshot(now)).catch(() => {});
  }
  // `maxAgeMs` lets a caller for whom a stale tree is WRONG rather than merely late ask for
  // less: a badge can live with two minutes, matching a Codex process to its rollout
  // cannot (a replaced agent would still be listed).
  const maxAge = Number.isFinite(opts.maxAgeMs) ? opts.maxAgeMs : SNAPSHOT_SERVE_MAX_MS;
  if (!_cache.procs || now - _cache.at >= maxAge) return null;
  return _cache.procs;
}
...
module.exports = {
  ...
  // #280
  snapshotNoWait, SNAPSHOT_SERVE_MAX_MS, INFLIGHT_MAX_MS,
  // #152 levels 2 and 3
