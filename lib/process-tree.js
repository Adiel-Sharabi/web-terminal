...
function _queryWindows() {
  return new Promise((resolve) => {
    ...
    // Win32_Process has already paid for the CIM session (see the table above). Its
    // failure is contained — `SilentlyContinue` plus a null guard, so a box whose perf
    // counters are broken still gets its process rows, which never depended on them.
    const script =
