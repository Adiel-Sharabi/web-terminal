...
// How old a snapshot `snapshotNoWait` will still serve (#280). Past this it answers null
// ("cannot tell") rather than a tree that may list shells which exited long ago. Only
// reached while queries keep FAILING: a healthy box refreshes every TTL. Eight TTLs
// covers a run of hung queries (each is killed at 15s) without reporting ghosts.
const SNAPSHOT_SERVE_MAX_MS = 120000;

// How long an in-flight query may hold the slot before a caller gives up on it and starts
// another (#280). execFile's own 15s timeout is supposed to end a hung PowerShell, but its
// callback waits for the pipes to close, and on office-tests a hung query took 25s to come
// back. Without this, a query that never calls back would pin `inflight` forever and no
// refresh would ever be tried again.
const INFLIGHT_MAX_MS = 30000;
...
let _cache = { at: 0, procs: null, perf: null, inflight: null, inflightAt: 0, prev: null, prevAt: 0, prevPerf: null };
...
async function snapshot(now = Date.now(), opts = {}) {
  ...
  if (_cache.inflight && Date.now() - _cache.inflightAt < INFLIGHT_MAX_MS) return _cache.inflight;
  ...
  // `mine` so a query abandoned past INFLIGHT_MAX_MS that answers LATE cannot overwrite
  // the newer query that replaced it, nor clear that query's slot.
  const mine = (_queryForTests || _queryWindows)().then((raw) => {
    if (_cache.inflight !== mine) return null;
    ...
    _cache = {
      at: startedAt, procs, perf, inflight: null, inflightAt: 0,
      prev: _cache.procs, prevAt: _cache.at, prevPerf: _cache.perf,
    };
    return procs;
  }).catch(() => { if (_cache.inflight === mine) _cache.inflight = null; return null; });
  _cache.inflight = mine;
  _cache.inflightAt = startedAt;
  return mine;
}

/**
 * The last good snapshot, returned AT ONCE, for callers inside a request (#280).
 *
 * `snapshot()` awaits the query, and the query is a PowerShell spawn. On office-tests
 * (2026-09-23) that spawn hung at startup, so `snapshot()` took 25s and returned null —
 * and a failed query is deliberately not cached, so every `GET /api/sessions` waited
 * out a fresh hung one while the box answered /api/version in 0.1s. A badge is not worth
 * a request; this serves what is known and lets the refresh land for the next poll.
 *
 * Starts (or joins) a refresh when the snapshot is past its TTL, never waits for it.
 * Null means "cannot tell": no snapshot yet, or the newest is older than
 * SNAPSHOT_SERVE_MAX_MS. Every caller already treats null that way.
 */
function snapshotNoWait(now = Date.now()) {
  if (!_cache.procs || now - _cache.at >= SNAPSHOT_TTL_MS) {
    Promise.resolve(snapshot(now)).catch(() => {});
  }
  if (!_cache.procs || now - _cache.at >= SNAPSHOT_SERVE_MAX_MS) return null;
  return _cache.procs;
}

/**
 * When the newest `exeName` under `rootPid` started, without waiting on a query (#280) -
 * what tells two Codex sessions in one folder apart (lib/codex-match.js).
 *
 * Returns { startMs, provisional }. It matches against ANY servable snapshot, because a
 * slightly old tree is still right for an agent that has not been restarted, and refusing
 * it made a HEALTHY box flip a session to its neighbour's rollout every ~23s (the snapshot
 * is only refreshed once a caller finds it past the TTL, and that caller got nothing).
 * `provisional` says the answer rests on a tree past the TTL, or on none: the caller may
 * serve it but should hold it only briefly, since a quit-and-restarted agent could still
 * be listed. A platform that can never take a snapshot is NOT provisional - it has no
 * better answer coming, and re-deriving on every call would only cost disk.
 */
function agentStartFromSnapshot(rootPid, exeName, now = Date.now()) {
  if (!_queryForTests && process.platform !== 'win32') return { startMs: null, provisional: false };
  const procs = snapshotNoWait(now);
  if (!procs) return { startMs: null, provisional: true };
  const proc = newestDescendantNamed(procs, rootPid, exeName);
  return {
    startMs: proc && Number.isFinite(proc.startMs) ? proc.startMs : null,
    provisional: now - _cache.at >= SNAPSHOT_TTL_MS,
  };
}
...
function _resetForTests() {
  _cache = { at: 0, procs: null, perf: null, inflight: null, inflightAt: 0, prev: null, prevAt: 0, prevPerf: null };
  _queryForTests = null;
...
module.exports = {
  ...
  // #280
  snapshotNoWait, agentStartFromSnapshot, SNAPSHOT_SERVE_MAX_MS, INFLIGHT_MAX_MS,
  // #152 levels 2 and 3
