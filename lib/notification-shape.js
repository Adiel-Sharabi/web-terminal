...
// `correctStaleStatus` does not time out a `waiting` session AT ALL since #230,
// against 5 minutes for a `working` one, so a benign notification misread as a
// permission ask parks a session on a false "waiting for permission" INDEFINITELY
// rather than for half a day — the cost of that mistake went UP when the backstop
// went away, so this gate matters more now, not less. Measure first, then decide from
...
// ANCHORED AND NOT GLOBAL. It is used with `.test()` on one token, and `.test()`
// on a `/g` regex advances `lastIndex` between calls — so a global flag here
// makes the answer depend on how many URLs were seen before it, which is a
// stateful bug in a module whose whole value is being pure.
const _URL_RE = /^[a-z][a-z0-9+.-]*:\/\//i;
...
function _looksLikePath(core) {
  ...
  if (/^~([\\/]|$)/.test(core)) return true;     // ~ or ~/… — but NOT `~50`
  // A BACKSLASH IS NOT ENGLISH. This clause exists because a token rule cannot
  // see across a space, and `C:\Program Files\secret.txt` is the ordinary
  // Windows case, not a corner: the drive half redacted and the FILENAME was
  // written to the log verbatim as a separate token. Raised in review, measured.
  if (core.includes('\\')) return true;
  const seps = (core.match(_SEP_RE) || []).length;
  if (!seps) return false;
  if (/^[.]{0,2}\//.test(core)) return true;     // /x  ./x  ../x
  if (seps >= 2) return true;                    // a/b/c — relative, but a path
  // One slash and a real file extension: `src/secret.env`, `Apps/secret.key`.
  // The extension is what separates it from prose — `and/or`, `24/7`, `n/a`,
  // `input/output` and `9/10` all carry one slash and no extension, and the
  // wording is the entire product of this function.
  return /^[^/]+\/[^/]*[.][A-Za-z0-9]{1,8}$/.test(core);
}
...
/// `v` prefix and `%` are included because `v1.72.3` and `87%` are quantities
/// too, and both were minting distinct shape keys — "87% complete" is an
/// entirely plausible wording.
function _looksLikeNumber(core) {
  return /^v?\d[\d.,:_%]*[a-z]{0,3}$/i.test(core);
}

/// An address is an identifier heading for a public issue. Cheap to catch, and
/// nothing else in this module would have.
const _EMAIL_RE = /^[^\s@]+@[^\s@]+\.[A-Za-z]{2,}$/;

// Anchored, whole-token forms of the two markers [redactNotificationMessage]
// below applies to the JOINED string. `_redactToken` needs its own copies
// because #199's default-deny fallback (see `_WORD_RE` and the comment on
// `_redactToken`) would otherwise beat that later pass to a standalone id/hex
// token and collapse it to the generic `<x>`, losing the more specific marker.
// Same 16+ threshold as `_LONG_HEX_RE` below, on purpose: keep the two in sync.
const _UUID_TOKEN_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const _HEX_TOKEN_RE = /^[0-9a-f]{16,}$/i;

/// #199 - what survives the DEFAULT-DENY fallthrough. A token survives
/// verbatim only if, after lead/trail punctuation and any label are stripped,
/// its core is letters optionally joined by an apostrophe (either kind),
/// underscore, hyphen or slash. See the rationale on `_redactToken` below:
/// this is the allowlist half of that change. The curly apostrophe is written
/// as a \u2019 escape, never as a literal byte, so the source file stays
/// ASCII-only (this repo has been burned by normalised non-ASCII before).
const _WORD_RE = /^[A-Za-z]+(?:['\u2019_/-][A-Za-z]+)*$/;
...
///
/// #199 - THE FALLTHROUGH IS DEFAULT-DENY, not a growing blocklist. It used to
/// be `return tok`: keep the token unless a classifier above recognises it as a
/// specific. That is a blocklist wearing a token rule's clothes, and it leaked a
/// real filename three distinct ways in one review: `_looksLikePath`'s own
/// comment already names the disease: "a blocklist of shapes leaks every shape
/// nobody thought of." The fallthrough now COMPOSES with every branch above
/// rather than replacing any of them: each classifier still runs first and
/// still wins, so a digit-bearing token one of them already understands still
/// gets its typed marker (`4096` -> `<n>`), and only the unrecognised remainder
/// falls to the generic `<x>`. `_WORD_RE` is the allowlist half; see its own
/// comment for exactly which characters survive and why.
///
/// Limits, stated rather than papered over (#199):
///  1. A bare identifier of letters and hyphens is indistinguishable from an
///     English word: a bare username or undotted hostname survives. That is
///     the price of keeping `read-only` and `web-terminal` on every line; there
///     is no syntactic rule that tells `webserver` apart from `approve`. A
///     DOTTED hostname still collapses to `<x>` (the dot is outside the class),
///     so only the bare undotted form gets through.
///  2. Allowing `/` re-admits two-segment relative tokens with no extension
///     (`src/lib`) - inherited from `_looksLikePath`'s own exemption above, not
///     introduced here.
///  3. Over-redaction is the STANDING, INTENDED trade: the next prose shape
///     nobody anticipated becomes `<x>` rather than leaking. `24/7` and `9/10`
///     now redact (a digit with no classifier to name it) where they used to
///     survive untouched, a deliberate consequence of "anything carrying a
///     digit is a specific," not a regression.
///  4. The real control is not the regex. These lines land in `logs/error.log`
///     on a machine the user owns; it becomes a leak only once that file is
///     pasted into a public issue. Defence in depth beats a fifth pattern.
///
/// `<id>` and `<hex>` are two markers this function did not previously
/// produce: they were left entirely to the whole-string `_UUID_RE` /
/// `_LONG_HEX_RE` passes in [redactNotificationMessage] below, which run AFTER
/// every token has already been through here. Under default-deny that is too
/// late: a standalone uuid or hex token (no slash, so `_looksLikePath` never
/// sees it) would already have been replaced with the generic `<x>` before
/// that second pass ever got to run, silently collapsing two of the six
/// markers this module exists to keep distinct (see the module-level "keep all
/// six markers" note). So the same two shapes are matched HERE too, per token,
/// ahead of the default-deny fallback and behind the number check (neither a
/// uuid's hyphens nor a hex run's letters ever satisfy `_looksLikeNumber`, so
/// ordering can't shadow it). The whole-string passes stay in place as a
/// second net for the one shape a single token can't tell apart from a word:
/// an all-letters hex run (`deadbeefcafebabe`, no digit anywhere).
function _redactToken(tok) {
  ...
  if (!core) return tok;
  // URL FIRST, and inside the token pipeline rather than as a pre-pass over the
  // whole string. As a pre-pass its `\S*` swallowed the punctuation after it —
  // `see (http://x.io/a) now` became `see ( <url> now` — which is the THIRD time
  // this module has produced that defect. One pipeline, punctuation handled once.
  // Ahead of the label split too, or `http:` reads as a label and the rest as a
  // path.
  if (_URL_RE.test(core)) return lead + '<url>' + trail;
  let label = '';
  if (!_DRIVE_RE.test(core)) {
    const m = _LABEL_RE.exec(core);
    if (m) { label = m[1]; core = core.slice(label.length); }
  }
  if (_EMAIL_RE.test(core)) return lead + label + '<email>' + trail;
  if (_looksLikePath(core)) return lead + label + '<path>' + trail;
  if (_looksLikeNumber(core)) return lead + label + '<n>' + trail;
  if (_UUID_TOKEN_RE.test(core)) return lead + label + '<id>' + trail;
  if (_HEX_TOKEN_RE.test(core)) return lead + label + '<hex>' + trail;
  return _WORD_RE.test(core) ? tok : lead + label + '<x>' + trail;
}
