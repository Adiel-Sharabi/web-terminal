...
function buildNtfyMessage(kind, { sessionName, serverName, reason, click } = {}) {
  const name = sessionName || 'session';
  const where = serverName ? `${serverName}: ${name}` : name; // "Office: DroneLocator"
  const base = { title: where, click: click || undefined };
  if (kind === 'approval') {
    return { ...base, message: reason || 'Claude needs your approval', priority: 5, tags: ['warning'] };
  }
  if (kind === 'apierror') {
    return { ...base, message: reason || 'API error', priority: 4, tags: ['rotating_light'] };
  }
  return { ...base, message: reason || 'Claude is done, waiting for input', priority: 3, tags: ['white_check_mark'] };
}
