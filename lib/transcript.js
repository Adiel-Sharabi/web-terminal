...
//  * `origin.kind === 'human'` — the other value is `peer` (34), another agent's
//    message wrapped in `<agent-message from="…">`, a signature classifyUserTurn
//    does not recognise. It reads as `human`, so admitting one would put another
//    agent's words in a "You" bubble. None of the 34 appears as a user turn
//    either, so a queued teammate message stays invisible; that is #250,
//    and its fix is a signature in `lib/user-turn.js`, not a wider gate here.
//
...
function _queuedPromptText(obj) {
  ...
  if (!a.origin || a.origin.kind !== 'human') return '';
  const p = a.prompt;
...
function parseTranscriptTurn(line) {
  ...
  if (obj.type === 'attachment') {
    ...
    //
    // So on every measured record this is REDUNDANT with the origin gate: the two
    // fields agree, 34 for 34 and 87 for 87. It is read anyway because it is the
    // record's own answer and the `role:user` branch obeys the same rule — but the
    // redundancy is the reason to say which way it fails. If a future version ever
    // marks a HUMAN-origin queued prompt `isMeta`, this publishes `typedText: ''`
    // and the prompt goes back to being invisible in chat. That is #249 itself
    // returning, so it is the first line to check if it ever does.
