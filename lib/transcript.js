...
// 800 sized for those. Kept deliberately modest: the bound that matters is the
// worst case, PQ_MAX_QUESTIONS x PQ_MAX_OPTIONS previews on one payload, and
// 2000 keeps that the same order as the label+description cost already there.
// Real prompts carry three or four options, so this is ~30 lines of code per
// option in practice — more than the overlay can show at once.
...
// that ordering is load-bearing: shaped previews are capped and label-less
// options are dropped, so recomputing it downstream would let a dropped preview
// silently flip the question to the compact layout — and the layout decides what
// every answer key means (#19/#84/#143).
