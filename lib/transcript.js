...
const fs = require('fs');
// The `role:user` rule, in its own dependency-free module precisely so this file
// can use it: requiring lib/recap.js here would close the cycle
// recap -> speech -> transcript (#149).
const { classifyUserTurn, typedTextOf } = require('./user-turn');
...
// render plain text, never move the cursor or set colors. The rule itself lives in
// `lib/ansi.js` — it is re-exported below, so `lib/speech.js` and
// `lib/transcript-codex.js` keep importing it from here exactly as before.
...
// C:\dev\acme\.claude-tmp -> C--dev-acme--claude-tmp.
...
// --- #249: a prompt sent while the agent is WORKING is not a `role:user` line -
...
//
// So a parser that reads only `user`/`assistant` lines never sees the prompt at
// all, and the ONLY thing showing it in the chat lens was the client's optimistic
// "Queued" echo (#31) — which renders after every transcript turn and after the
// working indicator, i.e. BELOW everything the agent produced in reply to it.
// That is the reported wrong order. Worse, the echo reconciles by comparing
// itself against a turn's `typedText` (#149): against a turn that does not exist
// it can never match, so once the session leaves `working` the echo's 90s safety
// timeout DELETES it and the prompt leaves chat entirely.
//
// Measured over 972 transcripts / 641 `queued_command` records: 80 of the 87
// human-typed ones have no matching `role:user` turn anywhere in their session,
// and exactly one has a match nearby — a genuine second send, after the turn was
// interrupted and the user re-typed the sentence 28s later. So emitting a turn
// here duplicates nothing that was measured.
//
// TWO GATES, both from that corpus, both narrowing rather than sniffing:
//  * `commandMode === 'prompt'` — the other value is `task-notification` (520 of
//    the 641), harness plumbing. Admitting them would add 520 muted rows
//    (`classifyUserTurn` reads one as `system`, so this is noise rather than a
//    mislabelled bubble), and 34 of them RESTATE a notification that already
//    arrives as a real `role:user` turn — matched on the `<task-id>` they share,
//    which is the only sound comparison: not one of the 520 is textually
//    identical to a user turn, so a prefix match answers whatever length you
//    pick and means nothing.
//
// THE SECOND GATE IS GONE, AND ITS JOB MOVED RATHER THAN VANISHED (#250). It read
// `origin.kind === 'human'`, excluding the 35 `peer` records — another agent's
// message, wrapped in `<agent-message from="…">` or `<cross-session-message …>`.
// Neither signature was known to `classifyUserTurn`, so such a prompt read as
// `human` and admitting one would have put a peer's words in a "You" bubble. The
// cost of refusing was that a teammate message sent mid-turn was invisible in the
// chat lens — #249's own symptom, for a different sender. Now that
// `lib/user-turn.js` knows both wrappers, the CLASSIFIER decides the bubble and
// this gate has nothing left to add: re-deriving "is this a peer" from `origin`
// beside a module that owns the question is the #227 mistake (sharing a module is
// not sharing an answer). `typedTextOf` yields '' for a teammate turn, so the
// "Queued" echo still cannot reconcile against somebody else's message.
//
// `origin.kind` is therefore READ BY NOBODY here. That is deliberate: the wrapper
// is in the prompt text itself, which is the thing the classifier gets to see, and
// a record whose origin says `peer` while its text carries no wrapper is one this
// module has never observed and would have no honest label for.
//
// The `rendered` field restates the prompt inside a `<system-reminder>` aimed at
// the agent. That is not what the user typed, so the turn is built from `prompt`
// alone — the same reason `typedText` exists rather than comparing raw text.
function _queuedPromptText(obj) {
  const a = obj && obj.attachment;
  if (!a || a.type !== 'queued_command') return '';
  if (a.commandMode !== 'prompt') return '';
  const p = a.prompt;
  if (typeof p === 'string') return p.trim();
  // A queued prompt carrying a pasted image arrives as content blocks. Text only
  // — the image is base64 bytes that must not be shipped to a client.
  if (Array.isArray(p)) {
    return p
      .filter(b => b && b.type === 'text' && typeof b.text === 'string')
      .map(b => b.text)
...
function parseTranscriptTurn(line) {
  ...
  if (obj.type === 'user' || role === 'user') {
    ...
    // #149 — what the user actually TYPED, alongside what is readable. The chat
    // lens's optimistic "Queued" echo has to recognise its own prompt coming back,
    // and neither `text` (reminder blocks stapled on, slash commands as a tag
    // trio) nor recap's `body` (which folds `<command-message>` prose in) can be
    // compared against. `''` for every turn the human did not type, which is what
    // stops an echo matching a teammate message. Derived from the already-capped
    // `text` so the two can never disagree about the same turn.
    // #163 — the record's OWN answer to "did a human write this?". A skill is
    // expanded into a `role:user` turn carrying the whole SKILL.md with no
    // wrapper, so no content signature can catch it; the harness marks such a
    // turn `isMeta`. This is the ONLY place with both the record and the rule, so
    // it is where the flag has to be read. The verdict is published as `userKind`
    // rather than kept private, because the companion carries a weaker twin of
    // the classifier and would otherwise need a fourth signature it cannot
    // derive — one owner for the rule, the clients read the answer.
    const isMeta = obj.isMeta === true;
    const opts = { isMeta };
    return {
      role: 'user',
      text,
      typedText: _capTurnText(typedTextOf(text, opts)),
      userKind: classifyUserTurn(text, opts).kind,
      toolUses: [],
      ts,
    };
  }
  // #249 — a prompt the user sent while the agent was working. See the note above
  // `_queuedPromptText`. Built into the SAME shape as a typed user turn, at its own
  // byte position, so it renders where it belongs (above the reply) and the chat
  // lens's "Queued" echo has something to reconcile against. Deliberately last:
  // every line that already parsed as a turn keeps parsing exactly as before.
  if (obj.type === 'attachment') {
    const text = _capTurnText(_queuedPromptText(obj));
    if (!text) return null;
    // #163's flag sits on the ATTACHMENT for this line shape, not on the record.
    // Measured over 641 `queued_command` records: top-level `isMeta` is set on
    // ZERO of them, `attachment.isMeta` on exactly the 34 peer ones and never on
    // the 87 human ones. Copying `obj.isMeta` from the `role:user` branch above —
    // where it IS the record's own field — would be a check that cannot fire.
    //
    // With the origin gate gone (#250) this flag is no longer redundant with
    // anything — it is the BACKSTOP under the wrapper signatures. It is set on all
    // 35 peer records and on none of the 96 human ones, so a peer message arriving
    // under some FOURTH wrapper nobody has seen classifies `meta` ("Injected",
    // muted, left-aligned) instead of `human`. Unlabelled is a poor outcome; a
    // "You" bubble for a peer's words is the bug, and this is what stops it being
    // reachable by an unrecognised shape.
    //
    // It is a backstop rather than the rule, and the tests say so: the peer specs
    // assert `teammate` SPECIFICALLY, never merely "not human". Asserting the
    // negation would pass on this flag alone and pin nothing about the signatures —
    // #249's vacuous-negative trap, in its new position.
    //
    // Which way it fails is unchanged: if a future version ever marks a
    // HUMAN-origin queued prompt `isMeta`, this publishes `typedText: ''` and the
    // prompt goes back to being invisible in chat. That is #249 itself returning,
    // so it is the first line to check if it ever does.
    const opts = { isMeta: obj.attachment.isMeta === true };
    return {
      role: 'user',
      text,
      typedText: _capTurnText(typedTextOf(text, opts)),
...
// A preview is a multi-line BLOCK (a code snippet, a diff, a mock-up), not a
// label or a one-line blurb, so it gets its own budget rather than sharing the
// 800 sized for those. Kept deliberately modest, and the number to weigh it
// against is the WORST CASE: PQ_MAX_QUESTIONS x PQ_MAX_OPTIONS on one payload
// takes the per-option budget from 1600 to 3600 bytes, so that ceiling moves
// ~384KB -> ~864KB — a 2.25x rise, not a wash. It is still bounded and still
// unreachable by any real prompt (three or four options, ~30 lines of code
// each), which is why the rise is acceptable rather than why it is small.
const PQ_PREVIEW_CAP = 2000;

function _pqCap(s, cap) {
  if (typeof s !== 'string') return '';
  const clean = stripAnsi(s);
...
//
// `hasPreview` is the one derived field here, and it is load-bearing rather than
// decorative: when ANY option of a question carries a `preview`, Claude renders
// that question SIDE-BY-SIDE (option list left, preview box right) instead of as
// a compact list, and the two layouts answer to different keys — a row digit
// SELECTS in the compact layout but only MOVES THE HIGHLIGHT in the side-by-side
// one. A client that cannot see the layout cannot build a correct key sequence,
// which is exactly how a two-question prompt whose first question had previews
// came to submit with that question blank. Layout is per-QUESTION, not
// per-prompt: measured 2026-08-03 against claude 2.1.220, a previewed Q1
// rendered side-by-side while the same prompt's preview-less multi-select Q2
// rendered compact.
//
// The preview BODY rides along too (#145), capped on its own budget. It used to
// be dropped here — correctly, while `hasPreview` existed only to pick a key
// sequence and nothing rendered the body. That stopped being true the moment
// the overlay could show it, and the side-by-side layout is precisely the one
// where the preview carries the decision: reported 2026-08-19, a three-option
// prompt whose labels were "Land #182" / "Play closed track upload" / "Leave
// it" kept every package name, snippet and blocker in the preview box, so the
// chat lens was strictly less informative than the terminal.
//
// `hasPreview` is derived from the RAW option list, NOT from the shaped one, and
// that ordering is load-bearing. The shaped list can LOSE a preview three ways —
// its option had no label and was filtered out, it fell past PQ_MAX_OPTIONS, or
// the body was a non-string / whitespace and so was never attached. (Capping is
// NOT one of them: _pqCap truncates, it can never empty a non-empty preview.)
// Recomputing the flag downstream would let any of those three silently flip the
// question to the compact layout — and the layout decides what every answer key
// MEANS (#19/#84/#143).
function _shapeQuestions(input) {
  ...
  for (const q of qs.slice(0, PQ_MAX_QUESTIONS)) {
    ...
    const kept = options.slice(0, PQ_MAX_OPTIONS);
    const shaped = {
      header: _pqStr(q.header),
      question: _pqStr(q.question),
      multiSelect: q.multiSelect === true,
      // `options`, NOT `kept` — the RAW list, before the PQ_MAX_OPTIONS slice.
      // Claude lays the question out from ALL of its options, so a preview
      // carried only by one past the cap still makes the selector side-by-side
      // while this flag would have called it compact. See the rule above; it
      // named the slice as one of the three ways the shaped list loses a
      // preview, and reading `kept` here was precisely that mistake.
      hasPreview: options.some(o => o && typeof o.preview === 'string' && o.preview.trim() !== ''),
      options: kept
        .map(o => {
          const opt = { label: _pqStr(o && o.label), description: _pqStr(o && o.description) };
          // Attached only when there is something to render, so a client never
          // has to tell "no preview" from "an empty one".
          const preview = _pqCap(o && o.preview, PQ_PREVIEW_CAP);
          if (preview.trim() !== '') opt.preview = preview;
          return opt;
        })
        .filter(o => o.label),
    };
...
module.exports = {
  ...
  // Exported so the spec asserts against the real budget instead of restating
  // the number — the one thing that would let the cap drift unnoticed (#145).
  PQ_PREVIEW_CAP,
  // Same reason (#143): the spec builds an option list one past the cap to pin
  // that `hasPreview` is read from the RAW options and not from the slice.
  PQ_MAX_OPTIONS,
};
