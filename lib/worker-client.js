...
class WorkerClient extends EventEmitter {
  constructor() {
    ...
    // The WORKER_VERSION the LIVE worker reported at handshake. The worker has always
    // sent it in the ping reply and the reply was thrown away, which made "did the hot
    // reload leave an old worker behind?" unanswerable from outside the box — the one
    // question a hot reload creates. Answering it took process start times cross-checked
    // against git reflog (#185). It is null whenever there is no worker attached, so it
    // can never name a version for a worker that has gone.
    this._workerVersion = null;
    this._exitHandlers = [];
    this._defaultTimeoutMs = 30000;
    // Per-session PTY_OUT listeners: Map<sessionId, Set<handler(Buffer)>>
    this._ptyOutHandlers = new Map();
    // Issue #11: cache of pre-computed 16-byte UUID buffers per session id
    // so sendPtyIn (per-keystroke on the web-side) skips the hex parse +
    // Buffer alloc on every call. Populated lazily on first sendPtyIn for
    // a given session id.
    this._idBytesCache = new Map();
  }
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (pong && pong.ok) {
          ...
          this._workerVersion = typeof pong.version === 'string' ? pong.version : null;
          return;
        }
        // A ping that RESOLVES but is not ok was the one path that neither closed nor
        // dropped the client: the next attempt overwrote `_client` while the first
        // stayed open and wired, so when that orphan eventually closed, its handler
        // cleared `_connected`/`_workerVersion` on a healthy connection and fired the
        // exit handlers — which end in process.exit(1). Tear it down like the catch does.
        try { if (this._client) this._client.close(); } catch { /* already gone */ }
        this._client = null;
      } catch (e) {
        lastErr = e;
  ...
  sendPtyIn(sessionId, data) {
    if (!this._client) return false;
    // Issue #11: reuse a cached 16-byte idBytes buffer when possible. Fall
    // back to the (slightly slower) uuid-parsing path on the first call for
    // a given session or if the id isn't a valid UUID — encodePtyIn will
    // throw in that case, which bubbles up to the caller exactly as before.
    let idBytes = this._idBytesCache.get(sessionId);
    if (!idBytes) {
      try {
        idBytes = ipc.uuidToBytes(sessionId);
      } catch {
        // Malformed id — defer to encodePtyIn which will throw the same way
        // the original code did.
        return this._client.send(ipc.encodePtyIn(sessionId, data));
      }
      if (idBytes.length !== 16) {
        // Not a valid UUID shape — behave like encodePtyIn (which would have
        // emitted a short/long uuid buffer and produced a malformed frame).
        // Fall back to the legacy path so any error surfaces identically.
        return this._client.send(ipc.encodePtyIn(sessionId, data));
      }
      this._idBytesCache.set(sessionId, idBytes);
    }
    return this._client.send(ipc.encodePtyInFromBytes(idBytes, data));
  }

  /** Drop a cached idBytes entry (call when a session is detached / ended). */
  forgetSession(sessionId) {
    this._idBytesCache.delete(sessionId);
    this._ptyOutHandlers.delete(sessionId);
  }
  ...
  /** WORKER_VERSION reported by the attached worker, or null when none is attached. */
  workerVersion() { return this._workerVersion; }
}
