...
function _rateLimits(rl) {
  if (!rl || typeof rl !== 'object') {
    return { fiveH: null, sevenD: null, fiveHResetAt: null, sevenDResetAt: null };
  }
  const entries = [rl.primary, rl.secondary].filter((e) => e && typeof e === 'object');
  let fiveH = null;
  let sevenD = null;
  // #142 — the WEEKLY reset, which the 5h-only reading could not express. The one
  // real Codex cap ever captured was on this window, so without it an observed cap can
  // neither expire nor name when it lifts.
  let sevenDResetAt = null;
  ...
  for (const e of entries) {
    if (_matchesWindow(e.window_minutes, FIVE_HOUR_MINUTES)) {
      ...
      sevenDResetAt = _resetAtMs(e.resets_at);
      matched = true;
    }
  }
  if (!matched) {
    ...
    sevenDResetAt = rl.secondary ? _resetAtMs(rl.secondary.resets_at) : null;
  }
  return { fiveH, sevenD, fiveHResetAt, sevenDResetAt };
}
...
// waits out the cap and takes another turn, Codex appends to the SAME file.
//
// COMPARED BY TIMESTAMP, NOT BY POSITION. The first cut walked backward and let
// iteration order stand in for "newer", which was wrong for the input this function is
// actually given: server.js's readTranscriptMetrics splices HEAD + TAIL for any rollout
// larger than its budget, so the oldest lines in the string are walked last. Found in
// review. Every rollout line carries a top-level ISO `timestamp` (verified against real
// local rollouts, e.g. {"timestamp":"2026-08-07T12:18:52.775Z","ordinal":379,...}), so
// the comparison is made on that and the splice stops mattering.
//
// That a same-turn `token_count` cannot land AFTER its own `task_complete` is MEASURED,
// not assumed — the whole rule dies if one does, because every cap would then read as
// superseded. Swept the 25 newest local rollouts, every `task_complete` in them: 23 were
// the LAST line of their file, and the other 2 were followed by
...
function findUsageCapAt(text) {
  ...
  let capAt = null;
  let latestTurnAt = null;

  for (const raw of text.split('\n')) {
    const line = raw.trim();
    if (!line) continue;
    let obj;
    try { obj = JSON.parse(line); } catch { continue; } // a truncated splice edge, etc.
    const p = obj && obj.payload;
    if (!p || typeof p !== 'object') continue;
    const ts = Date.parse(obj.timestamp);
    if (!Number.isFinite(ts)) continue;             // unplaceable in time; ignore it

    if (obj.type === 'turn_context'
        || (obj.type === 'response_item' && p.type === 'message' && p.role === 'user')
        || (obj.type === 'event_msg' && p.type === 'token_count')) {
    ...
    if (obj.type === 'event_msg' && p.type === 'task_complete'
        ...
        && p.error.codex_error_info === 'usage_limit_exceeded') {
      if (capAt === null || ts > capAt) capAt = ts;
    }
  }

  if (capAt === null) return null;
  if (latestTurnAt !== null && latestTurnAt > capAt) return null; // a later turn ran
  return capAt;
}
...
function parseMetricsFromTail(text) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (!usage && obj.type === 'event_msg' && p.type === 'token_count' && p.info) {
      ...
      const { fiveH, sevenD, fiveHResetAt, sevenDResetAt } = _rateLimits(p.rate_limits);
      ...
      if (ctx !== null || fiveH !== null || sevenD !== null) {
        usage = { ctx, fiveH, sevenD, fiveHResetAt, sevenDResetAt };
      }
    }
  ...
  return {
    ...
    sevenDResetAt: usage ? usage.sevenDResetAt : null,
    model,
    effort,
    capObservedAt,
  };
