...
function parseSessionMeta(line) {
  ...
  return {
    ...
    // 'user' for a real session, 'subagent' for a Task-style child. A subagent rollout
    // also carries forked_from_id / parent_thread_id, and its `session_id` points at the
    // PARENT — so it is NOT a session's conversation and must never be resolved as one.
    // Seen live on Office: two of the three rollouts in one cwd were subagents.
    threadSource: typeof p.thread_source === 'string' ? p.thread_source : '',
  };
}
