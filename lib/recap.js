...
const { classifyUserTurn, USER_KINDS } = require('./user-turn');
...
function isHumanPrompt(turn) {
  return Boolean(turn) && turn.role === 'user' &&
    classifyUserTurn(turn.text).kind === USER_KINDS.HUMAN;
}
...
function findHumanPromptIndex(turns) {
  ...
  for (let i = list.length - 1; i >= 0; i--) {
    if (isHumanPrompt(list[i])) return i;
  }
  return -1;
}
...
function buildRecap(turns, opts = {}) {
  ...
  const promptIdx = findHumanPromptIndex(list);
  let promptBody = '';
  let promptAt = null;
  if (promptIdx >= 0) {
    promptBody = classifyUserTurn(list[promptIdx].text).body;
    promptAt = list[promptIdx].ts || null;
  }
