...
const PROVIDERS = Object.freeze({
  ...
  codex: Object.freeze({
    ...
    resolveTranscript(session, io) {
      const cwd = session && session.cwd;
      if (!cwd) return '';
      // processStartMs, when known, turns "newest rollout in this cwd" into "the rollout
      // this process created" — see lib/codex-match.js.
      return findRolloutForCwd(cwd, io, { processStartMs: session.processStartMs });
    },
