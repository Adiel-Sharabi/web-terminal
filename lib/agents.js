...
const PROVIDERS = Object.freeze({
  ...
  codex: Object.freeze({
    ...
    // Codex's TUI interrupts on Esc too ("Esc to interrupt"). It has no hooks wired up here
    // at all, so its status never leaves 'active' today — but the policy belongs with the
    // agent, not in the worker, so it is declared rather than assumed.
    interrupt: Object.freeze({ onEscape: true }),
  }),
});
