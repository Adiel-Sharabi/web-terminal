...
const PROVIDERS = Object.freeze({
  ...
  claude: Object.freeze({
    ...
    // Claude's TUI has no burst→paste detector: `text\r` in one write submits, which is
    // why both clients send the prompt and its CR as ONE frame (#44 — a delayed CR can
    // be dropped if the socket dies in the gap). The gap below is used only where the
    // worker itself submits a prompt (auto-recovery replay).
    submit: Object.freeze({ gapMs: 60, crBurstsAsPaste: false }),
  }),
...
// plain shell, which has no provider at all) keeps the historical behaviour: one write,
// and the worker's own replay CR 60ms behind it.
const DEFAULT_SUBMIT = Object.freeze({ gapMs: 60, crBurstsAsPaste: false });
