...
const PROVIDERS = Object.freeze({
  ...
  claude: Object.freeze({
    ...
    usageLimitPrompt: Object.freeze({
      // Capture group 1 is the option's OWN rendered number, so we answer the row
      // that literally says "stop and wait" rather than a position we assumed. If
      // the menu is ever reordered this still picks the right one; `answer` is only
      // the fallback for a render that somehow carries no number.
      pattern: /(\d)\.\s*Stop and wait for limit to reset/i,
      answer: '1',
    }),
