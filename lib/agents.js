...
//                            bracketAbove    a plain-text submit body longer than this is
//                                            wrapped in bracketed paste before it is
//                                            written, so the TUI is TOLD it is a paste
//                                            instead of inferring it. Undeclared (and 0)
//                                            = never, which is the behaviour every agent
//                                            had before the field existed. Measured in
//                                            BYTES, not characters. See lib/submit-frames.js
//                                            `bracketLongBody` for the measurement.
//
...
// #190 measured this marker; #210 gave it a SECOND consumer, so it gets a name.
//
// U+276F followed by U+00A0 NO-BREAK SPACE: what Claude's composer writes, and what its
// folder-trust dialog does NOT -- that dialog positions every word with CHA and emits no
// spaces of any kind. The measurement table lives on `readiness` in the provider below,
// beside the alternatives it killed.
//
// TWO JOBS, ONE FACT. The worker scans the PTY BYTE STREAM with it to decide a submit may
// go out (#147); the companion tests the RENDERED SCREEN with it to decide whether the
// chat lens's tail strip has anything worth showing (#210). Both ask the same question --
// is the composer on screen -- so a second copy for the second asker is exactly the drift
// this file exists to prevent. That is why it is PUBLISHED on GET /api/agents rather than
// retyped in Dart, the same arrangement the label and the colour already use.
//
// The rendered path was MEASURED, not assumed (2026-09-02): writing caret+NBSP into the
// companion's vendored xterm and reading the row back through `BufferLine.getText()`
// returns code units [10095, 160], and `caret + U+0020` still fails to match. So the
// discrimination survives a round trip through a terminal buffer, which is the only
// reason a byte-stream marker is allowed to answer a screen question at all.
//
// KEEP THIS LINE ASCII-ONLY. A literal U+00A0 is invisible in a diff and is normalised to
// an ordinary space in transit. tests/composer-marker.spec.js pins the behaviour rather
// than the spelling, and its NEGATIVE case is the load-bearing one.
const CLAUDE_COMPOSER = new RegExp('\u276f\u00a0');
...
const PROVIDERS = Object.freeze({
  ...
  claude: Object.freeze({
    ...
    //
    // `bracketAbove` is the OTHER half of the same fact, and it exists because splitting
    // the CR rescues the SUBMIT while leaving the BODY to the TUI's own paste inference.
    // #213 — a ~1500-character DICTATED prompt (reported 2026-09-02) looked complete in the
    // compose box and reached the terminal as its tail alone. Dictation emits no line
    // breaks, and `buildComposeSubmission` brackets on exactly one predicate — does the
    // buffer contain a newline — so a dictated paragraph is the one long prompt that
    // still goes to the TUI raw. Measured (probe-paste-single-line.js, claude 2.1.251,
    // verdict from the transcript): unbracketed is whole at 288/488/988, NOT SUBMITTED
    // once at 588, starts no turn at 1088, and at 1588 loses its first 1024 characters
    // EXACTLY, twice, the survivor beginning mid-token. Bracketed is whole at every one
    // of 588/1588/2588, and probe-paste-claude.js measures that shape whole to 41,899.
    //
    // 512 rather than the 988 that measured clean: 588 was clean on one run and lost its
    // submit on another, so the band below the hard 1024 boundary is already unreliable,
    // and the threshold belongs under the flakiness rather than under the cliff. It is
    // still far above anything typed by hand — `y`, `1`, `continue` and an ordinary
    // one-line question all stay byte-identical to today.
    submit: Object.freeze({ gapMs: 150, crBurstsAsPaste: true, bracketAbove: 512 }),
    // #179 — how long to wait for PROOF that a submitted prompt was actually taken,
    // before telling the client it was not. Claude alone declares this, because Claude
    // alone has a channel that can prove it: its hooks. The worker starts the clock when
    // the submit CR goes out and stops it on the first hook of any kind — a session
    // doing anything keeps firing them, while one parked on `/usage` fires none.
    //
    // There is deliberately no "blocked" MARKER here. #179 proposed alt-screen and said
    // to measure it first; measured on claude 2.1.250, not one blocking state emitted
    // `ESC[?1049h` — not `/usage`, not the slash menu, not Agent View. Nothing else
    // separates them either, so the outcome is verified instead of the cause predicted.
    // See lib/submit-confirm.js and scripts/rig/probe-altscreen-block.js.
    //
    // 8s is generous by design: `UserPromptSubmit` reaches us over loopback in well
    // under a second, and the gates in lib/submit-confirm.js already refuse every case
    // where a slow answer would be legitimate (a queued prompt, a slash command).
    submitConfirm: Object.freeze({ timeoutMs: 8000 }),
    ...
    //
    // #190 — THE BARE CARET WAS NOT SPECIFIC ENOUGH, and the cost was not
    // theoretical: Claude's FOLDER-TRUST dialog draws U+276F as its own
    // selection cursor. So the latch flipped on a session sitting at a selector,
    // the session was published ready, and the client was cleared to submit into
    // it. That selector's default row is `No, exit` and its footer is `Enter to
    // confirm`, so the submit's trailing CR does not merely get eaten — it
    // CONFIRMS EXIT and drops the agent back to bash. Measured, verdicts from
    // hasTrustDialogAccepted on disk plus "did a turn start", never the screen.
    //
    // The comment here used to defer this, asking for "a fresh rig measurement
    // of what that frame prints". That measurement is done
    // (scripts/rig/probe-trust-prompt.js, claude 2.1.251). The composer writes
    // the caret followed by U+00A0 NO-BREAK SPACE; the trust dialog writes the
    // caret followed immediately by CHA (`ESC[4G`), because that dialog emits no
    // spaces of any kind — it positions every single word with CHA. Five bytes,
    // so it sits well inside lib/agent-ready.js's CARRY_BYTES and needs no
    // change there.
    //
    //   case                      caret  caret+NBSP  chevrons  rule  Try  agents
    //   composer 120 cols          yes      YES        yes      yes  yes   yes
    //   composer 52 cols           yes      YES        yes      yes  yes    no
    //   composer --permission-mode
    //     default / plan           yes      YES         NO      yes  yes   yes
    //   trust dialog 120 / 52      yes       no         no      yes   no    no
    //   bare shell                  no       no         no       no   no    no
    //
    // Every other candidate died in that table, which is why none of them is
    // here: `⏵⏵` is MODE-dependent (absent in default mode, and plan mode prints
    // a different glyph); the `───` rule is drawn by the trust dialog too; `Try`
    // is the empty-composer placeholder and rotates its wording; `agents` is
    // TRUNCATED AWAY at 52 columns, i.e. a phone-only regression of exactly the
    // #146 shape; and `effort` matched only via this fleet's own statusLine
    // script, not anything Claude owns.
    //
    // A RESTORED session still latches — measured, because getting this backwards
    // would be worse than the bug it fixes: `claude --resume <id>` printed the
    // marker ~1.1s after the launch write at both 120 and 52 columns, confirmed
    // through a real cold restart of the rig and not merely the command string.
    // A resume against a MISSING conversation prints `No conversation found with
    // session ID: …` and returns to the shell with no caret of any kind, so the
    // 45s fallback still covers it — unchanged in width by this fix, since the
    // bare caret did not match there either.
    //
    // WRITTEN AS ESCAPES, AND THE SOURCE OF THIS LINE MUST STAY ASCII-ONLY. A
    // literal U+00A0 is invisible in a diff and gets normalised to an ordinary
    // space in transit: four separate attempts to write the escape form into the
    // probe silently came back as a literal. Nothing non-ASCII means nothing to
    // normalise. tests/composer-marker.spec.js pins the behaviour rather than the
    // spelling, and its NEGATIVE case is the load-bearing one — if normalisation
    // hits the rule and the test together, the positive case passes because both
    // sides became ordinary spaces.
    //
    // What no unit test can catch: a future claude release changing the glyph or
    // the spacing. That needs a rig re-probe, not an assertion.
    readiness: Object.freeze({ composer: CLAUDE_COMPOSER }),
    ...
    // being capped looks like AND how to answer it — a session that cannot even go
    // idle until the selector is answered, which is what needs rescuing.
    //
    // #142 — Codex declares false PERMANENTLY, not "for now": its own cap event has
    // since been captured too (see the Codex provider's `autoResume` comment below),
    // and it does not block on anything answerable — the turn ends and the composer
    // returns on its own. There is nothing stuck for an auto-resume to rescue, so
    // arming it would only spend quota on work the user may already consider done.
    ...
    // #190 - the STARTUP SELECTOR FAMILY that parks a session before its composer
    // exists. The rule is pure and lives in lib/blocking-prompt.js, whose header
    // carries the capture, the CHA problem and the two-tier argument; only the
    // agent-specific strings are here.
    //
    // MEASURED off a real PTY, claude 2.1.268, 2026-09-11 (probe-trust-prompt.js
    // capture, with WT_TRUST_PROBE_PARENT pointed at a directory with no trusted
    // ancestor). Unchanged from the 2.1.251 capture, and NOT suppressed by
    // `--dangerously-skip-permissions`.
    //
    // `footer` is the FAMILY's commit affordance, not one dialog's wording, and it
    // is the only wording this registry declares for recognising a block. Three
    // members are known to render this way - folder trust, the external-CLAUDE.md
    // imports selector, and the usage-limit selector next door - and the shape gate
    // in lib/blocking-prompt.js covers whichever of them is on screen, plus
    // whatever Claude adds next. That is deliberate: a member nobody has measured
    // still gets REPORTED, because reporting costs a refused submit while missing
    // it costs the prompt.
    //
    // `known` is the much narrower list of members whose answer we are willing to
    // TYPE, and it names THE LABEL OF THE ROW THAT WILL BE PRESSED rather than a
    // title near it - matching exactly what is acted on. The arrows are then
    // counted off the render (lib/blocking-prompt.js), never assumed, for the same
    // reason `matchUsageLimitPrompt` reads its digit off the menu: the default row
    // here is `No, exit`, so a fixed key sequence against a reordered render would
    // not merely pick wrong, it would kill the agent.
    //
    // TYPING ANY OF THIS IS OFF BY DEFAULT - `autoAnswerBlockingPrompt` in
    // config.json, default false (see docs/CONFIGURATION.md). Trusting a folder is
    // precisely what gates Claude executing what is in it, so the decision to
    // automate it belongs to the person whose folder it is.
    blockingPrompts: Object.freeze({
      footer: /Enter to confirm/,
      known: Object.freeze([
        Object.freeze({ id: 'folder-trust', accept: /^Yes, I trust this folder$/ }),
      ]),
      // Between each key, and before the confirming CR. The measured-good value:
      // scripts/rig/probe-trust-prompt.js drove `down-enter` with 600ms between the
      // keys and the dialog was answered. A gap is not optional - every agent TUI
      // folds one read into a paste and swallows a trailing CR (#55), and a
      // selector confirmed by Enter is the same hazard with a worse outcome. 150ms
      // (Claude's `submit.gapMs`) may well do; it has not been measured HERE, and
      // an unmeasured value in a path that presses `Yes, I trust this folder` is
      // not where to save 450ms.
      answerGapMs: 600,
    }),
  ...
  codex: Object.freeze({
    ...
    // NO `bracketAbove` here, and for the same reason as the readiness marker above:
    // nobody has run probe-paste-single-line.js against a Codex rollout. Its paste-burst
    // handling is its own (paste_burst.rs), so Claude's 1024-character cliff is not
    // evidence about it in either direction. Undeclared means the body is written exactly
    // as it always has been — today's behaviour, unchanged. Adapt the probe's verdict to
    // read a rollout instead of a .jsonl, sweep it, then declare the number here.
    ...
    // #142 — DECIDED, not deferred: Codex's cap has now been captured, and it is a
    // shape auto-resume has nothing to rescue.
    //
    // A real cap event (Office, 2026-08-02T20:26:58.735Z — see lib/metrics-codex.js
    // findUsageCapAt) shows Codex does not block on anything answerable the way
    // Claude does. Claude's cap presents a SELECTOR ("Stop and wait for limit to
    // reset") that leaves the session sitting there, unable to go idle until someone
    // answers it — that stuck state is exactly what auto-resume exists to rescue.
    // Codex's turn simply ENDS: the rollout records `event_msg -> task_complete` with
    // `error.codex_error_info: "usage_limit_exceeded"`, and the composer returns.
    // There is nothing stuck. Typing `continue` into a session that already handed
    // control back to the user would spend quota on work they may already consider
    // finished — the precise harm #138 exists to prevent, now confirmed rather than
    // merely feared.
    //
    // The detectable signal is that rollout error, not a PTY-scanned prompt — Codex
    // has no prompt to scan for. readMetrics (above) already recovers fiveH /
    // fiveHResetAt AND, since #142, capObservedAt (findUsageCapAt), so the badge and
    // wait period render and the row correctly shows "held" — lib/usage-limit.js
    // `usageLimitState` treats a rollout-observed cap as the same direct-observation
    // class as Claude's observed prompt. `arm` stays false because there is nothing
    // to arm FOR: no answerable state to resolve by writing a keystroke. This is not
    // "unmeasured, revisit later" — it is measured, and the measurement is why arming
    // stays off.
...
//
// `bracketAbove` is deliberately NOT defaulted, and that asymmetry is the point: splitting
// a CR is invisible to anything that does not need it, while wrapping a body in
// `ESC[200~…ESC[201~` is only correct for a reader that understands the sequence. A real
// plain shell whose readline has bracketed paste off would take the markers as literal
// text — `[200~` pasted in front of the command — so the one agent that measured the
// need declares it, and everything else keeps the bytes it has always been given.
const DEFAULT_SUBMIT = Object.freeze({ gapMs: 150, crBurstsAsPaste: true });

// #179 — NOT declared by default, and not by Codex. A submit can only be verified where
// something can vouch for it, and Codex's OSC 9 channel cannot: it fires on exactly two
// occasions, an approval and a FINISHED turn, so "a turn began" is unreachable through it
// by construction (see statusFromOutput below). Undeclared means the worker never watches,
// which is today's behaviour exactly — the same convention `readiness` follows.
const DEFAULT_SUBMIT_CONFIRM = null;
...
/**
 * #179 — the submit-confirmation policy for a session's agent, or null when the agent
 * declares none (Codex, plain shells, anything unknown). A null answer means "never
 * watch", so an agent added without thinking about this is silent rather than noisy.
 */
function submitConfirmPolicy(agentId) {
  return (isKnownAgent(agentId) && PROVIDERS[agentId].submitConfirm) || DEFAULT_SUBMIT_CONFIRM;
}
...
// #190 - the startup selector family this agent parks on, or null when it declares
// none.
//
// A plain shell (null) and every unknown agent say NO, and Codex says no too. That
// is the standing convention rather than caution: Claude's behaviour is not evidence
// about Codex in either direction, its startup dialogs have not been captured, and
// an unmeasured marker is exactly what #143 shipped. Undeclared means never - today's
// bytes exactly, for everything but Claude.
function blockingPromptsFor(agentId) {
  const p = isKnownAgent(agentId) ? PROVIDERS[agentId] : null;
  return (p && p.blockingPrompts) || null;
}
...
function listProviders() {
  return AGENT_IDS.map((id) => {
    ...
    return {
      ...
      label: p.label,
      color: p.color,
      // #210 - the composer marker, so a client can tell "the terminal is merely sitting
      // at the prompt" from "something else is on screen". Published as the regex SOURCE
      // for the same reason the label and the colour are published: this registry is the
      // one owner, and a provider that gains a marker has to reach both clients with no
      // release.
      //
      // A provider that declares none publishes null, and a client holding null must FAIL
      // OPEN -- show the terminal. Showing it is never worse than the behaviour before
      // #210; hiding it on a guess is how a session parked on a dialog goes invisible.
      // `.source` carries the PATTERN, not the flags - and that is fine only for as long
      // as no provider declares a flagged marker. Both consumers rebuild it flagless (the
      // spec with `new RegExp(...)`, the companion with Dart's `RegExp(...)`), so a
      // future /i or /m would be silently dropped at the client while still applying in
      // the worker. Publish flags too, or keep markers flagless.
      composerMarker:
        p.readiness && p.readiness.composer ? p.readiness.composer.source : null,
    };
  });
}
...
module.exports = {
  usageLimitPromptFor, armsAutoResume, blockingPromptsFor,
  getAdapter,
  isKnownAgent,
