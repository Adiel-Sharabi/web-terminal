...
function usageLimitState({ metrics, enabled, delayMs, now, observedBlockAt, canArm, armedByWorker }) {
  ...
  return {
    ...
    // "resumes 21:51" from the 19:27 cancel until that moment passed - two and a half
    // hours naming a time nothing was scheduled for - and the badge lying is what
    ...
    // truth since #138 (pty-worker.js `autoResumeArmed: !!s._autoResumeTimer`); it was
    // simply never read - one grep hit repo-wide, the definition.
    //
