...
/**
 * Is auto-resume on for a server whose config has never said either way? (#137, #240)
 *
 * Here for the same reason the delay above is: BOTH processes need it and they must
 * agree. `pty-worker.js` reads it to decide whether a timer may arm at all, and
 * `server.js` fills it in on `GET /api/config` for a key the file omits.
 *
 * WHAT WENT WRONG: THE API MISREPORTED ITS OWN SERVER. server.js answered `false` -
 * #69's opt-in default, which #137 flipped and that line never followed - for a feature
 * the worker was running as `true` and `docs/CONFIGURATION.md` documents as `true`. An
 * endpoint that reports a setting the server does not have is a defect on its own,
 * whoever reads it.
 *
 * AND `PUT /api/config` REPLACES RATHER THAN MERGES, so anything that echoes the served
 * object back writes that misreport to disk. In this repo the only things that do are
 * two test helpers (`tests/exclusive-viewer.spec.js`, `tests/keep-sessions-open.spec.js`),
 * which is exactly how it reached the gitignored `config.test.json` - a file no checkout
 * restores, so one interrupted run disabled arming permanently on that machine while CI,
 * having no such file, stayed green (#240).
 *
 * NOT a claim about the shipped clients, which was the first draft of this comment and
 * was wrong: `app.html` and `lobby.html` both build a FRESH object from their form fields
 * and never mention this key at all, and the companion only ever GETs. Their bug is the
 * opposite one - they DROP keys they do not render (#242).
 *
 * A shared constant rather than a drift check: there is one definition, so the two cannot
 * come apart at all. `docs/CONFIGURATION.md` documents this value - keep it in step there,
 * since prose cannot import.
 */
const AUTO_RESUME_ON_RESET_DEFAULT = true;
