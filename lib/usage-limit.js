 ...
 */
/**
 * Is auto-resume on for a server that has never said either way? (#137, #240)
 *
 ...
 *
 * THE SERVED DEFAULT IS A WRITE. A settings client edits one field and PUTs the whole
 * object back (`PUT /api/config` replaces rather than merges), so whatever the GET
 * hands out is what an ordinary round-trip PERSISTS. While these two disagreed, that
 * round-trip wrote an opt-out nobody chose: server.js was still serving #69's opt-in
 * `false` after #137 turned the feature on. Under the test suite the file it landed in
 * is the gitignored `config.test.json`, which no checkout restores, so one interrupted
 * run disabled arming permanently on that machine and CI never saw it (#240).
 *
 * A shared constant rather than a drift check: there is one definition, so the two
 * cannot come apart at all. `docs/CONFIGURATION.md` documents this value — keep it in
 * step there, since prose cannot import.
 */
