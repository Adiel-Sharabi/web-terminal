...
// config.toml contains e.g. `\\?\C:\dev\MyProject`). Normalise all of that before
...
function findRolloutForCwd(cwd, io, opts = {}) {
  ...
  const claimed = opts.claimedIds instanceof Set ? opts.claimedIds : null;
  const inCwd = [];
  for (const c of candidates) {
    let head;
    try { head = io.readFirstLine(c.path); } catch { continue; }
    const meta = parseSessionMeta(head);
    if (!meta || !sameCwd(meta.cwd, cwd, platform)) continue;
    // A SUBAGENT rollout is not a session's conversation — it is a child of one, and its
    // session_id points at the parent. Ranking these as candidates meant a session's chat
    // could show a Task's transcript. Two of the three rollouts in one real cwd were
    // subagents, so this is not a corner case.
    if (meta.threadSource === 'subagent') continue;
    // A conversation another session has already CLAIMED (reported through
    // scripts/codex-notify.js) belongs to that session. Serving it here is precisely the
    // defect being fixed — one session showing another's work — so it is skipped even
    // though it would otherwise win on mtime.
    if (claimed && meta.id && claimed.has(meta.id)) continue;
    inCwd.push(c);
  }
