...
| `autoAnswerBlockingPrompt` | Yes | Let the worker **type the answer** to a recognised startup selector — today only Claude's folder-trust dialog, answered with `Yes, I trust this folder` (#190). Default **false**, deliberately: trusting a folder is precisely what gates the agent reading, editing and executing what is in it, so the decision belongs to whoever owns the folder. **Turning this off does not make the dialog invisible** — such a session is reported as blocked either way, its compose bar refuses to submit into the selector and says why, and the chat lens's tail strip shows the screen. The keys pressed are read off the render (the row whose label matches), never a fixed sequence, because the dialog's default row is `No, exit` and a confirming Enter on it kills the agent. Override per-process with the `WT_AUTO_ANSWER_BLOCKING_PROMPT` env var (`0`/`1`). |
