...
| `lib/scrollback-window.js` | The backward walk through a session's scrollback, anchored on CONTENT rather than a byte offset (#178). The worker trims the buffer's head on every PTY write, so `offset 0` means a different byte on every request while `total` sits pinned at the cap — an absolute offset is not a position. Keeps a head anchor of what is on screen, over-fetches by the anchor's length, cuts where the anchor reappears, escalates 4K→16K→64K on a periodic region, and STOPS rather than guess. Dual-mode: required by the spec, and served to `app.html` so both run the same file |
...
| `lib/submit-confirm.js` | #179 - the pure rules for VERIFYING a submit landed, rather than predicting whether the TUI could take one. `shouldWatchSubmit` (the gates: the provider must declare a window, the session must have delivered a hook, the write must come from a client, the line must not be a slash command, the session must not already be `working`), `confirmsSubmit`, and `createSubmitLineTracker` (what the TUI input line holds, since a live `/`-line is streamed as you type and its submit frame is a bare CR). Its header records the measurement that ruled alt-screen out as a blocked-state marker |
| `lib/terminal-size.js` | #146/#59 - one PTY has one size and many viewers may disagree. `negotiateSize` picks the SMALLEST **active** viewer that has actually reported a size (a terminal smaller than its viewer wastes space; one larger is unreadable), returning null when nobody has an opinion so a REST client never resizes a live agent. Applied in `server.js` on every resize, mode change and socket close |
